...
const STATUS_CONFIG: Record<
    OrderStatus,
    { label: string; sublabel: string; color: string; bg: string; ring: string; emoji: string }
> = {
    pending:     { label: "Menunggu Konfirmasi",  sublabel: "Pesanan kamu sedang menunggu dikonfirmasi oleh outlet.",         color: "text-amber-600",   bg: "bg-amber-50 dark:bg-amber-950/30",    ring: "bg-amber-400",   emoji: "⏳" },
    confirmed:   { label: "Mencari Kurir",        sublabel: "Outlet sudah konfirmasi! Sedang mencari kurir terdekat.",        color: "text-blue-600",    bg: "bg-blue-50 dark:bg-blue-950/30",      ring: "bg-blue-400",    emoji: "🔍" },
    preparing:   { label: "Sedang Dipersiapkan",  sublabel: "Kurir ditemukan! Outlet sedang menyiapkan pesananmu.",           color: "text-orange-600",  bg: "bg-orange-50 dark:bg-orange-950/30",  ring: "bg-orange-400",  emoji: "👨‍🍳" },
    ready:       { label: "Pesanan Siap",         sublabel: "Pesananmu sudah siap dan menunggu dijemput kurir.",              color: "text-green-600",   bg: "bg-green-50 dark:bg-green-950/30",    ring: "bg-green-400",   emoji: "✅" },
    on_delivery: { label: "Dalam Perjalanan",     sublabel: "Kurir sedang dalam perjalanan menuju lokasimu. Tunggu sebentar!", color: "text-cyan-600",    bg: "bg-cyan-50 dark:bg-cyan-950/30",      ring: "bg-cyan-400",    emoji: "🛵" },
    delivered:   { label: "Pesanan Tiba!",        sublabel: "Pesananmu sudah tiba. Selamat menikmati! 🎉",                   color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-950/30",ring: "bg-emerald-400", emoji: "🎊" },
};
        ...
        <div className="relative flex items-center justify-center w-32 h-32">
            ...
            {[...Array(5)].map((_, i) => (
                <motion.span
                    ...
                    transition={{ duration: 1, delay: i * 0.1, repeat: Infinity, repeatDelay: 1.5, ease: "easeOut" }}
                />
            ))}
        </div>
    );
        ...
        <div className="relative flex items-center justify-center w-36 h-36">
            {particles.map((_, i) => (
                <motion.span
                    ...
                    transition={{ duration: 1.2, delay: i * 0.05, repeat: Infinity, repeatDelay: 1.2, ease: "easeOut" }}
                />
            ))}
            <motion.span
                className="text-5xl select-none z-10"
                ...
                {STEPS.map((step, idx) => {
                    ...
                    return (
                        <div key={step.key} className="relative z-10 flex flex-col items-center gap-1">
                            ...
                            <span className={`text-[9px] font-medium whitespace-nowrap ${active ? "text-foreground" : "text-muted-foreground/50"}`}>
                                {step.label}
                            </span>
                        </div>
                    );
        ...
        <div className="min-h-[60vh] flex flex-col items-center justify-center px-4 py-12 gap-8">
            ...
            >
                ...
                <div className="text-center space-y-1.5">
                    ...
                    <p className="text-sm text-muted-foreground leading-snug">{cfg.sublabel}</p>
                </div>
                ...
                <div className="w-full rounded-xl bg-background/60 border border-border/40 px-4 py-3 space-y-1 text-xs text-muted-foreground">
                    ...
                    <div className="flex justify-between">
                        ...
                        <span className="font-bold text-foreground truncate max-w-[140px] text-right">{outletName}</span>
                    </div>
                </div>
