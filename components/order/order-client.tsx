...
import {
  BasketSheetContent,
  type CartItem,
} from '@/components/order/basket-sheet-content';
...
export function OrderClient({
  ...
  if (errorFetchOutlet) {
    console.error(
      '[OrderClient] Failed to fetch outlet:',
      errorFetchOutlet.message,
    );
  }
            ...
            <div className="flex items-end gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <span
                    className={`px-2.5 py-0.5 rounded-full text-[10px] font-black ${outlet?.isOpen ? 'bg-green-400/90 text-green-900' : 'bg-red-400/90 text-red-900'}`}
                  >
                  ...
                  {outlet?.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-0.5 rounded-full bg-white/20 backdrop-blur-sm text-[10px] font-bold text-white"
                    >
                      {tag}
                    </span>
                  ))}
            ...
            <SheetHeader>
              <SheetTitle className="font-black text-xl flex items-center gap-2">
                ...
                <Badge variant="secondary" className="ml-auto">
                  {wishlistStore.items.length} item
                </Badge>
              </SheetTitle>
              <SheetDescription className="sr-only">
                Produk yang disimpan untuk dibeli nanti
              </SheetDescription>
            </SheetHeader>
          ...
          >
            <SheetHeader>
              <SheetTitle className="font-black text-xl flex items-center gap-2">
                ...
                <Badge variant="secondary" className="ml-auto ml-5">
                  {cartTotal} item
                </Badge>
              </SheetTitle>
              <SheetDescription className="sr-only">
