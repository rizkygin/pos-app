            ...
            <div className="px-5 py-4 flex flex-col gap-4 flex-1">
                ...
                <div className="flex flex-col gap-1 text-sm">
                    ...
                    <div className="flex items-start gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">{order.outletAddress}</span>
                    </div>
                </div>
...
const MY_STATUS_MAP: Record<string, { label: string; headerBg: string; headerBorder: string; textColor: string; badgeBg: string }> = {
    preparing:   { label: "Diproses",    headerBg: "bg-blue-50 dark:bg-blue-950/20",    headerBorder: "border-blue-100 dark:border-blue-900/30",    textColor: "text-blue-700 dark:text-blue-400",    badgeBg: "bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400" },
    ready:       { label: "Siap Ambil",  headerBg: "bg-emerald-50 dark:bg-emerald-950/20", headerBorder: "border-emerald-100 dark:border-emerald-900/30", textColor: "text-emerald-700 dark:text-emerald-400", badgeBg: "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400" },
    on_delivery: { label: "Diantar",     headerBg: "bg-orange-50 dark:bg-orange-950/20",  headerBorder: "border-orange-100 dark:border-orange-900/30",  textColor: "text-orange-700 dark:text-orange-400",  badgeBg: "bg-orange-100 dark:bg-orange-900/40 text-orange-600 dark:text-orange-400" },
    confirmed:   { label: "Dikonfirmasi", headerBg: "bg-amber-50 dark:bg-amber-950/20",  headerBorder: "border-amber-100 dark:border-amber-900/30",    textColor: "text-amber-700 dark:text-amber-400",    badgeBg: "bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400" },
};
    ...
    return (
        <>
            <motion.div
                ...
                className="rounded-2xl border bg-background shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col"
            >
                <div className={`flex items-center justify-between px-5 py-3 ${s.headerBg} border-b ${s.headerBorder}`}>
                    <div className="flex items-center gap-2">
                        <span className={`font-mono font-black text-sm ${s.textColor}`}>
                            #{order.orderId.slice(-8).toUpperCase()}
                        </span>
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${s.badgeBg}`}>
                            {s.label}
                        </span>
                    </div>
                    <div className="flex items-center gap-2">
...
export function CourierLobby({ courierId }: { courierId: number }) {
    ...
    const tabs = [
        { id: "available" as Tab, label: "Tersedia",     count: available.orders.length, badgeColor: "bg-violet-500" },
        { id: "mine"      as Tab, label: "Pesanan Saya", count: mine.orders.length,      badgeColor: "bg-blue-500"   },
    ];
