...
const Page = async ({ params }: { params: Promise<{ order_id: string }> }) => {
  ...
  const res = await serverFetch(`/api/admin/orders/${order_id}`);
  if (res.status === 404) notFound();
  if (!res.ok) redirect('/dashboard');

  const { order, items } = (await res.json()) as {
    order: any;
    items: { detail_id: number; quantity: number; note: string | null; summary_price: string; product_name: string; price: string; category: string; unit: string }[];
  };
