...
export const PURCHASE_STATUS_STYLE: Record<PurchaseRow["status"], string> = {
...
export const PURCHASE_STATUS_LABEL: Record<PurchaseRow["status"], string> = {
  draft: "Draft",
  posted: "Diposting",
  partial: "Sebagian",
  paid: "Lunas",
...
export function getPurchaseColumns(opts: {
  busyId: number | null;
  ...
  return [
    ...
    {
      id: "supplier",
      header: () => <span className="px-3">Supplier</span>,
      cell: ({ row }) => (
        <span className="text-muted-foreground">{row.original.supplier_name || row.original.party_name || "—"}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${PURCHASE_STATUS_STYLE[row.original.status]}`}>
          {PURCHASE_STATUS_LABEL[row.original.status]}
        </span>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex justify-end gap-1">
            ...
            {inv.status !== "void" && (
              <a
                href={`/dashboard/invoice/purchase/${inv.id}/print`}
                target="_blank"
