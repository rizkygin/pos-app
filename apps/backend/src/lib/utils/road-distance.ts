 ...
 */
export const DETOUR_FACTOR = 1.7;

export type Coord = { lat: number; lon: number };

export type DistanceResult = {
  km: number;
  /** Driving time estimate, minutes. Null when falling back to straight line. */
  minutes: number | null;
  /** 'road' = real routing; 'straight' = haversine, OSRM unavailable. */
 ...
 */
export function billableKm(d: DistanceResult): number {
  return d.source === "road" ? d.km : d.km * DETOUR_FACTOR;
}
 ...
 */
export async function roadTable(
  origin: Coord,
  destinations: Coord[],
): Promise<DistanceResult[]> {
  const straight = (): DistanceResult[] =>
    destinations.map((d) => ({
      km: haversineKm(origin.lat, origin.lon, d.lat, d.lon),
      minutes: null,
  ...
  try {
    ...
    const res = await fetch(
      `${OSRM_BASE_URL}/table/v1/driving/${coords}?sources=0&annotations=distance,duration`,
      { signal: AbortSignal.timeout(OSRM_TIMEOUT_MS), headers: { "User-Agent": USER_AGENT } },
    );
    ...
    if (json.code !== "Ok" || !json.distances?.[0]) return straight();
...
export type RouteResult = DistanceResult & {
  /** The drawn path as [lat, lon] pairs. Two points when falling back. */
 ...
 */
export async function roadRoute(from: Coord, to: Coord): Promise<RouteResult> {
  ...
  try {
    ...
    const res = await fetch(
      `${OSRM_BASE_URL}/route/v1/driving/${path}?geometries=geojson&overview=full`,
      { signal: AbortSignal.timeout(OSRM_TIMEOUT_MS), headers: { "User-Agent": USER_AGENT } },
    );
    if (!res.ok) return straight();
    ...
    if (!route || typeof route.distance !== "number" || !coords?.length) return straight();
...
const cache = new Map<string, { value: DistanceResult; expires: number }>();
 ...
 */
export async function roadDistance(from: Coord, to: Coord): Promise<DistanceResult> {
  const straight: DistanceResult = {
    km: haversineKm(from.lat, from.lon, to.lat, to.lon),
    minutes: null,
    source: "straight",
  ...
  try {
    ...
    const route = json.code === "Ok" ? json.routes?.[0] : undefined;
    if (!route || typeof route.distance !== "number") return straight;
