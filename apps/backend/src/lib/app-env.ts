...
type EnvUrls = {
  /** Browser-facing frontend origins. First one wins where a single URL is needed. */
  frontendOrigins: string[];
  /** Browser-facing backend origin. */
  apiUrl: string;
  /**
   * Cookie scope. Undefined = host-only, which is what a laptop needs and what
   * *.up.railway.app would need too — that suffix cannot carry a shared cookie.
   */
  cookieDomain: string | undefined;
  cookieSecure: boolean;
};

const URLS: Record<AppEnv, EnvUrls> = {
  production: {
...
export const API_URL = process.env.PUBLIC_API_URL ?? derived.apiUrl;

export const AUTH_URL = process.env.BETTER_AUTH_URL ?? API_URL;
...
console.log(
  `[env] APP_ENV=${APP_ENV} api=${API_URL} frontend=${FRONTEND_ORIGINS.join(",")} ` +
    `cookieDomain=${COOKIE_DOMAIN ?? "(host-only)"} secure=${COOKIE_SECURE}`,
);
