...
async function main() {
  ...
  `);
    for (const m of APPLIED) {
        const content = (0, fs_1.readFileSync)(`./drizzle/${m.tag}.sql`).toString();
        const hash = (0, crypto_1.createHash)('sha256').update(content).digest('hex');
        const existing = await pool.query('SELECT id FROM public.__drizzle_migrations WHERE hash = $1', [hash]);
        if (existing.rowCount === 0) {
            await pool.query('INSERT INTO public.__drizzle_migrations (hash, created_at) VALUES ($1, $2)', [hash, m.when]);
