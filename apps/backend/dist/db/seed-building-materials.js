...
async function main() {
    ...
    const [outlet] = await index_1.db
        .select({ id: schema_1.outletsTable.id, name: schema_1.outletsTable.name })
    ...
    const existingGroups = await index_1.db
        .select({ id: schema_1.menuGroupsTable.id, name: schema_1.menuGroupsTable.name, sort: schema_1.menuGroupsTable.sort_order })
        .from(schema_1.menuGroupsTable)
        .where((0, drizzle_orm_1.eq)(schema_1.menuGroupsTable.outlet_id, outletId));
    ...
    for (const name of new Set(CATALOGUE.map((i) => i.group).filter(Boolean))) {
    ...
    for (const item of CATALOGUE) {
        ...
        const [existing] = await index_1.db
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.productsTable.outlet_id, outletId), (0, drizzle_orm_1.eq)(schema_1.productsTable.product_name, item.name)))
            .limit(1);
        if (existing) {
            await index_1.db.update(schema_1.productsTable).set(values).where((0, drizzle_orm_1.eq)(schema_1.productsTable.id, existing.id));
            updated += 1;
        }
        else {
            await index_1.db.insert(schema_1.productsTable).values({ id: (0, crypto_1.randomUUID)(), ...values });
            created += 1;
        }
        const label = deliverable
            ? `fixed ${price}`
            : `band ${lowest}–${highest} (haul room ${highest - lowest}/${item.unit})`;
        console.log(`  ${deliverable ? "kurir " : "sopir "} ${item.name.padEnd(32)} ${(item.group ?? "-").padEnd(18)} ${label}`);
    }
    ...
    const features = await (0, outlet_features_1.recalcOutletFeatures)(outletId);
