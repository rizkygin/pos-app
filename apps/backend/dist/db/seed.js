...
const main = async () => {
    ...
    for (const user of usersToSeed) {
        ...
        try {
            ...
            if (user.name === 'Admin') {
                await index_1.db.insert(schema_1.adminsTable).values({
                    user_id: res.user.id,
                    email: user.email,
                    name: 'Admin Rizky',
                });
            ...
            if (user.name === 'Pelanggan Offline' || user.name === 'Rizky Online') {
                await index_1.db.insert(schema_1.customersTable).values({
                    user_id: res.user.id,
                });
            }
            else if (user.name === 'owner_test') {
                await index_1.db.insert(schema_1.outletsTable).values({
                    ...
                    lat: String(coords_1.DEFAULT_COORDS.lat),
                    lon: String(coords_1.DEFAULT_COORDS.lon),
                });
                await addProductstoOutlets(res.user.id);
            }
            else if (user.name === 'courier_test') {
                await index_1.db.insert(schema_1.couriersTable).values({
                    user_id: res.user.id,
...
const addProductstoOutlets = async (user_id) => {
    const outlet = await index_1.db.query.outletsTable.findFirst({
        where: (0, drizzle_orm_1.eq)(schema_1.outletsTable.user_id, user_id),
    });
    if (!outlet) {
    ...
    for (const product of productsToSeed) {
        try {
            ...
            await index_1.db.insert(schema_1.productsTable).values(product);
        }
        catch (error) {
...
const seedCategories = async () => {
    const [cashInCategory] = await index_1.db
        .select({ count: (0, drizzle_orm_1.count)(schema_2.cashInCategoryTable.id) })
        .from(schema_2.cashInCategoryTable);
    const [cashOutCategory] = await index_1.db
        .select({ count: (0, drizzle_orm_1.count)(schema_2.cashOutCategoryTable.id) })
        .from(schema_2.cashOutCategoryTable);
    if (cashInCategory.count > 0) {
        cashflow_categories_1.CATEGORY_IN.forEach(async (category) => {
            await index_1.db.insert(schema_2.cashInCategoryTable).values({
                category: category,
            });
        });
    }
    if (cashOutCategory.count > 0) {
        cashflow_categories_1.CATEGORY_OUT.forEach(async (category) => {
            await index_1.db.insert(schema_2.cashOutCategoryTable).values({
                category: category,
            });
        });
    }
...
const seedScheduleProductAds = async () => {
    ...
    const [existing] = await index_1.db
        .select({ count: (0, drizzle_orm_1.count)(schema_2.scheduleProductAdsTable.id) })
        .from(schema_2.scheduleProductAdsTable);
    if (existing.count > 0) {
        console.log('Schedule product ads already seeded, skipping.');
        return;
    }
    await index_1.db
        .insert(schema_2.scheduleProductAdsTable)
