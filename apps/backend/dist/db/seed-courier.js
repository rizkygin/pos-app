...
const UPLOAD_DIR = node_path_1.default.join(process.cwd(), "uploads", "couriers");
 ...
 */
function faceArt(ink, turn, showFeatures) {
...
function documentArt(ink, heading) {
    return `
    ...
    ${[430, 470, 510, 550]
        .map((y, i) => `<rect x="256" y="${y}" width="${230 - i * 34}" height="14" rx="7" fill="#1c1917" opacity="0.45"/>`)
        .join("")}`;
}
/** Two wheels and a body. Enough to tell the four angles apart. */
function vehicleArt(ink, label) {
    return `
...
function slotSvg(groupId, kind, label, title) {
    const { bg, bg2, ink } = PALETTE[groupId];
    const art = groupId === "face"
        ? faceArt(ink, kind === "face_right" ? 34 : kind === "face_left" ? -34 : 0, kind !== "face_back")
        : groupId === "papers"
...
async function findOrCreateUser() {
    ...
    await index_1.db
        ...
        .where((0, drizzle_orm_1.eq)(schema_1.usersTable.id, res.user.id));
...
async function main() {
    ...
    let [courier] = await index_1.db
        ...
        .limit(1);
    if (!courier) {
        [courier] = await index_1.db
            .insert(schema_1.couriersTable)
            .values({
            user_id: userId,
            vehicle_plate: PLATE,
            vehicle_type: "motorcycle",
            verification_status: STATUS,
            verification_note: STATUS === "rejected"
                ? "Foto wajah tampak kiri masih memakai topi, dan foto STNK buram."
                : null,
        })
            .returning({ id: schema_1.couriersTable.id });
    ...
    if (DOCS === "none") {
        ...
        const slots = courier_documents_1.COURIER_DOCUMENT_GROUPS.flatMap((g) => g.kinds.map((k) => ({ ...k, groupId: g.id, groupTitle: g.title })));
        ...
        for (const slot of wanted) {
            ...
            await (0, sharp_1.default)(Buffer.from(svg)).webp({ quality: 85 }).toFile(node_path_1.default.join(UPLOAD_DIR, filename));
            const image = `${URL_PREFIX}${filename}`;
            await index_1.db
                .insert(schema_1.courierDocumentsTable)
                .values({ courier_id: courier.id, kind: slot.kind, image })
                .onConflictDoUpdate({
