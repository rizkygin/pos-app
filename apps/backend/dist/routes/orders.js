 ...
 */
async function resolveLane(items) {
    const ids = [...new Set(items.map((i) => i.product_id))];
    if (ids.length === 0)
        return "delivery";
    const [bulky] = await db_1.db
        .select({ id: schema_1.productsTable.id })
        .from(schema_1.productsTable)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.inArray)(schema_1.productsTable.id, ids), (0, drizzle_orm_1.eq)(schema_1.productsTable.courier_deliverable, false)))
        .limit(1);
 ...
 */
async function materialsFeeCap(orderId) {
    const rows = await db_1.db
        .select({
        quantity: schema_1.orderDetailsTable.quantity,
        lowest: schema_1.productsTable.lowest_price,
        highest: schema_1.productsTable.highest_price,
    })
        .from(schema_1.orderDetailsTable)
        .innerJoin(schema_1.productsTable, (0, drizzle_orm_1.eq)(schema_1.productsTable.id, schema_1.orderDetailsTable.product_id))
...
function deliveryFeeFromDistance(km) {
    ...
    if (km > MAX_KM)
        throw new Error(`Jarak pengiriman melebihi batas maksimum (${MAX_KM} km)`);
    if (km <= BASE_KM)
        return BASE_FEE;
...
async function computeDeliveryFee(userId, outletId) {
    const [[userLoc], [outlet]] = await Promise.all([
        db_1.db
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.locationsTable.user_id, userId), (0, drizzle_orm_1.eq)(schema_1.locationsTable.is_default, true)))
            .limit(1),
        db_1.db
            .select({ lat: schema_1.outletsTable.lat, lon: schema_1.outletsTable.lon })
            .from(schema_1.outletsTable)
            .where((0, drizzle_orm_1.eq)(schema_1.outletsTable.id, outletId))
            .limit(1),
    ]);
    if (!userLoc)
    ...
    const distance = await (0, road_distance_1.roadDistance)({ lat: parseFloat(outlet.lat), lon: parseFloat(outlet.lon) }, { lat: parseFloat(userLoc.lat), lon: parseFloat(userLoc.lon) });
    return deliveryFeeFromDistance((0, road_distance_1.billableKm)(distance));
}
/**
 ...
 */
async function assertWithinDeliveryRange(userId, outletId) {
    const [[userLoc], [outlet]] = await Promise.all([
        db_1.db
            .select({ lat: schema_1.locationsTable.lat, lon: schema_1.locationsTable.lon })
            .from(schema_1.locationsTable)
    ...
    if ((0, road_distance_1.billableKm)(distance) > MAX_DELIVERY_KM) {
...
async function orderRoutes(app) {
    ...
    app.get("/api/orders/delivery-fee", async (request, reply) => {
        ...
        const outletId = Number(request.query.outlet_id);
        if (!outletId)
            return reply.send({ error: "Outlet tidak ditemukan" });
        try {
    ...
    app.post("/api/orders/create", async (request, reply) => {
        ...
        try {
            ...
            const [[customer], delivery_fee, [outlet]] = await Promise.all([
                ...
                db_1.db
                    .select({ user_id: schema_1.outletsTable.user_id, name: schema_1.outletsTable.name })
                    .from(schema_1.outletsTable)
                    .where((0, drizzle_orm_1.eq)(schema_1.outletsTable.id, data.outlet_id))
                    .limit(1),
            ]);
            if (!customer)
            ...
            await db_1.db.transaction(async (tx) => {
                ...
                if (data.items.length > 0) {
                    await tx.insert(schema_1.orderDetailsTable).values(data.items.map((item) => ({
                        order_id: orderId,
            ...
            if (outlet?.user_id) {
                ...
                (0, push_1.sendPushToUser)(outlet.user_id, {
                    title: isService ? "Permintaan layanan baru" : "Pesanan baru masuk",
                    body: `${session.user.name ?? "Pelanggan"} · ${itemCount} item${isService ? "" : ` · ${orderId.slice(-8).toUpperCase()}`}`,
    ...
    app.post("/api/orders/cancel-by-customer", async (request, reply) => {
        ...
        const [customer] = await db_1.db
            ...
            .limit(1);
        if (!customer)
            return reply.status(403).send({ success: false, error: "Customer not found" });
        ...
        const res = await db_1.db
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.customer_id, customer.id), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, order.status)))
    ...
    app.post("/api/orders/confirm", async (request, reply) => {
        ...
        const confirmed = await db_1.db
            .update(schema_1.ordersTable)
            .set({ status: "confirmed", updatedAt: new Date() })
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.outlet_id, outlet.id), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "pending")))
            .returning({ id: schema_1.ordersTable.id, fulfillment: schema_1.ordersTable.fulfillment });
        ...
        if (confirmed[0]?.fulfillment === "delivery") {
            try {
                await (0, dispatch_1.dispatchNextOffer)(orderId);
            }
            catch (err) {
    ...
    app.post("/api/orders/reject-by-owner", async (request, reply) => {
        ...
        if (!trimmedReason)
            return reply.status(400).send({ success: false, error: "Alasan penolakan wajib diisi" });
        const outlet = await outletForOrders(session.user.id, request);
        if (!outlet)
            return reply.status(403).send({ success: false, error: "Not an owner" });
        const res = await db_1.db
            ...
            .returning({ id: schema_1.ordersTable.id });
        if (res.length === 0)
            return reply.status(400).send({ success: false, error: "Order tidak dapat ditolak" });
        return reply.send({ success: true });
    });
    ...
    app.post("/api/orders/mark-ready", async (request, reply) => {
        ...
        await db_1.db
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.outlet_id, outlet.id), 
        ...
        (0, drizzle_orm_1.or)((0, drizzle_orm_1.isNotNull)(schema_1.ordersTable.courier_id), (0, drizzle_orm_1.inArray)(schema_1.ordersTable.fulfillment, ["service", "materials"])), (0, drizzle_orm_1.or)((0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "confirmed"), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "preparing"))));
        return reply.send({ success: true });
    });
    ...
    app.post("/api/orders/confirm-pickup", async (request, reply) => {
        ...
        await db_1.db
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.outlet_id, outlet.id), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "ready")));
        return reply.send({ success: true });
    });
        ...
        try {
            await db_1.db.transaction(async (tx) => {
                const [idCategory] = await tx
                    ...
                    .where((0, drizzle_orm_1.eq)(schema_1.cashInCategoryTable.category, cashflow_categories_1.CATEGORY_IN[0]))
                    .limit(1);
                ...
                const amount = await tx
                    .select({
                    sum: (0, drizzle_orm_1.sql) `sum(cast(${schema_1.orderDetailsTable.summary_price} as numeric))`,
                })
                    .from(schema_1.orderDetailsTable)
                    .where((0, drizzle_orm_1.eq)(schema_1.orderDetailsTable.order_id, orderId));
                const cashIn = await tx
                    ...
                    .returning({ id: schema_1.cashInDetailTable.id });
                ...
                const [courier] = await tx
                    ...
                    .limit(1);
                if (!courier)
                    throw new Error("Not a courier");
                await tx
                    .update(schema_1.orderDetailsTable)
                    .set({ status: "checkout" })
                    .where((0, drizzle_orm_1.eq)(schema_1.orderDetailsTable.order_id, orderId));
                await tx
                    .update(schema_1.ordersTable)
                    .set({ status: "delivered", updatedAt: new Date() })
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.courier_id, courier.id), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "on_delivery")));
            });
            return reply.send({ success: true });
        }
        catch (error) {
    ...
    async function requireOwnerOutlet(request, reply) {
        const session = await auth_1.auth.api.getSession({ headers: (0, web_headers_1.toWebHeaders)(request.headers) });
        if (!session?.user) {
    ...
    app.post("/api/orders/confirm-service", async (request, reply) => {
        ...
        try {
            ...
            const [row] = await db_1.db
                ...
                .from(schema_1.ordersTable)
                .innerJoin(schema_1.orderDetailsTable, (0, drizzle_orm_1.eq)(schema_1.orderDetailsTable.order_id, schema_1.ordersTable.id))
                .innerJoin(schema_1.productsTable, (0, drizzle_orm_1.eq)(schema_1.productsTable.id, schema_1.orderDetailsTable.product_id))
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.outlet_id, ctx.outlet.id)))
                .limit(1);
            if (!row)
                return reply.status(404).send({ success: false, error: "Order tidak ditemukan" });
            if (row.fulfillment !== "service")
                return reply.status(400).send({ success: false, error: "Bukan order layanan" });
            ...
            await db_1.db.transaction(async (tx) => {
                ...
                await tx
                    .update(schema_1.ordersTable)
                    .set({ status: "confirmed", updatedAt: new Date() })
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "pending")));
            });
            return reply.send({ success: true });
        }
        catch (error) {
    ...
    app.get("/api/orders/:orderId/materials-quote", async (request, reply) => {
        ...
        const [row] = await db_1.db
            ...
            .leftJoin(schema_1.locationsTable, (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.locationsTable.user_id, schema_1.customersTable.user_id), (0, drizzle_orm_1.eq)(schema_1.locationsTable.is_default, true)))
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.outlet_id, ctx.outlet.id)))
            .limit(1);
        if (!row)
            return reply.status(404).send({ success: false, error: "Order tidak ditemukan" });
        ...
        const distance = hasCoords
            ? await (0, road_distance_1.roadDistance)({ lat: parseFloat(row.outletLat), lon: parseFloat(row.outletLon) }, { lat: parseFloat(row.lat), lon: parseFloat(row.lon) })
            : null;
        return reply.send({
            success: true,
            ...
            driveMinutes: distance?.minutes == null ? null : Math.round(distance.minutes),
            dropoff: hasCoords
                ? {
                    label: row.label,
                    address: row.address,
    ...
    app.post("/api/orders/confirm-materials", async (request, reply) => {
        ...
        if (!ctx)
            return;
        const { orderId, delivery_fee } = request.body ?? {};
        ...
        try {
            const [row] = await db_1.db
                .select({ status: schema_1.ordersTable.status, fulfillment: schema_1.ordersTable.fulfillment })
                .from(schema_1.ordersTable)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.outlet_id, ctx.outlet.id)))
                .limit(1);
            if (!row)
    ...
    app.post("/api/orders/schedule-service", async (request, reply) => {
        ...
        if (!ctx)
            return;
        const { orderId, scheduled_at, discount_amount, discount_percent } = request.body ?? {};
        ...
        try {
            ...
            const [{ sum: agreed } = { sum: 0 }] = await db_1.db
                .select({ sum: (0, drizzle_orm_1.sql) `coalesce(sum(cast(${schema_1.orderDetailsTable.summary_price} as numeric)), 0)` })
                .from(schema_1.orderDetailsTable)
                .where((0, drizzle_orm_1.eq)(schema_1.orderDetailsTable.order_id, orderId));
            let discount = 0;
            if (discount_percent != null && discount_percent !== "") {
                ...
                if (Number.isFinite(pct) && pct > 0)
                    discount = Math.round((Number(agreed) * pct) / 100);
            }
            ...
            if (discount > Number(agreed))
                discount = Number(agreed);
            const res = await db_1.db
                .update(schema_1.ordersTable)
                .set({ status: "preparing", scheduled_at: when, discount_amount: String(discount), updatedAt: new Date() })
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.outlet_id, ctx.outlet.id), (0, drizzle_orm_1.eq)(schema_1.ordersTable.fulfillment, "service"), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "confirmed")))
                .returning({ id: schema_1.ordersTable.id });
            if (res.length === 0)
                return reply.status(400).send({ success: false, error: "Order tidak dapat dijadwalkan" });
    ...
    app.post("/api/orders/close-service", async (request, reply) => {
        ...
        const res = await db_1.db
            .update(schema_1.ordersTable)
            .set({ status: "on_delivery", updatedAt: new Date() })
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.outlet_id, ctx.outlet.id), (0, drizzle_orm_1.eq)(schema_1.ordersTable.fulfillment, "service"), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "ready")))
            .returning({ id: schema_1.ordersTable.id });
        if (res.length === 0)
            return reply.status(400).send({ success: false, error: "Order tidak dapat ditutup" });
        return reply.send({ success: true });
    });
        ...
        try {
            ...
            await db_1.db.transaction(async (tx) => {
                const [order] = await tx
                    .select({
                    outlet_id: schema_1.ordersTable.outlet_id,
                    discount: schema_1.ordersTable.discount_amount,
                })
                    .from(schema_1.ordersTable)
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.customer_id, customer.id), (0, drizzle_orm_1.eq)(schema_1.ordersTable.fulfillment, "service"), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "on_delivery")))
                    .limit(1);
                if (!order)
                    throw new Error("Order tidak dapat diselesaikan");
                ...
                const [idCategory] = await tx
                    ...
                    .limit(1);
                if (!idCategory)
                    throw new Error("Kategori kas masuk belum ada");
                const cashIn = await tx
                    .insert(schema_1.cashInDetailTable)
                ...
                await tx
                    .update(schema_1.ordersTable)
                    .set({ status: "delivered", updatedAt: new Date() })
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "on_delivery")));
            });
            return reply.send({ success: true });
        }
        catch (error) {
    ...
    app.post("/api/orders/dispatch-materials", async (request, reply) => {
        ...
        const res = await db_1.db
            .update(schema_1.ordersTable)
            .set({ status: "on_delivery", updatedAt: new Date() })
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.outlet_id, ctx.outlet.id), (0, drizzle_orm_1.eq)(schema_1.ordersTable.fulfillment, "materials"), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "ready")))
            .returning({ id: schema_1.ordersTable.id });
        if (res.length === 0)
        ...
        try {
            ...
            await db_1.db.transaction(async (tx) => {
                const [order] = await tx
                    ...
                    .from(schema_1.ordersTable)
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.ordersTable.id, orderId), (0, drizzle_orm_1.eq)(schema_1.ordersTable.customer_id, customer.id), (0, drizzle_orm_1.eq)(schema_1.ordersTable.fulfillment, "materials"), (0, drizzle_orm_1.eq)(schema_1.ordersTable.status, "on_delivery")))
                    .limit(1);
                if (!order)
                    throw new Error("Order tidak dapat diselesaikan");
                ...
                const net = Math.max(0, Number(goods) + Number(order.delivery_fee ?? 0) - Number(order.discount ?? 0));
                const [idCategory] = await tx
                    .select({ id: schema_1.cashInCategoryTable.id })
                    .from(schema_1.cashInCategoryTable)
                    .where((0, drizzle_orm_1.eq)(schema_1.cashInCategoryTable.category, cashflow_categories_1.CATEGORY_IN[0]))
